...
public class QuadStation extends PApplet {
    public static void main(String args[]) {
       PApplet.main(new String[] { "com.jyoder.quadstation.QuadStation" });
    }
...
int Centerlimits[]  = {1100,1800}; // Endpoints of ServoCenterSliders 

cGraph g_graph;
int windowsX    = 1200;       int windowsY    = 540+tabHeight;
...
/**
 * Build single msp without payload
 * @param msp msp int
 * @return List<Byte> of this msp
 */
...
/**
 * Build multiple msp without payload
 * @param msps
 * @return byte array of multiple msps
 */
List<Byte> requestMSP (int[] msps) {
...
/**
 * Build an MSP with payload
 * @param msp int, the MSP
 * @param payload Character[], the payload
 * @return
 */
...
/**
 * "sends the complete byte sequence in one go"
 * just sends a byte array out the serial port apparently
 * @param msp List of bytes (?)
 */
...
public void evaluateCommand(byte cmd, int dataSize) {
  ...
  switch(icmd) {
    ...
    case MSP_ANALOG:
        ...
        controlP5.get("Cell Voltage").setValue(bytevbat / 30.0f);
        break;
    case MSP_RC_TUNING:
...
int time,time2,time3,time40,time5,time6;
int last1HzTime = 0;
boolean enabled = false;
boolean wasEnablePressed = false;
int errorCount = 0;
int successCount = 0;
  ...
  if (init_com==1 && graph_on==1) {
    ...
    if (!toggleRead && !toggleWrite && !toggleSetSetting ) {
      ...
      if(GamePad.isConnected()) {
        boolean enablePressed = GamePad.getButton(Identifier.Button._7);
  
        if(enablePressed && (enablePressed != wasEnablePressed)) {
          enabled = !enabled;
        }
        wasEnablePressed = enablePressed;
      	int[] rcPayload = {
      	     (int)Math.round(GamePad.getAxis(GamePad.ROLL) * 500 + 1500), // roll
      	     (int)Math.round(GamePad.getAxis(GamePad.PITCH) * 500 + 1500), //pitch
      	     (int)Math.round(GamePad.getAxis(GamePad.YAW) * 500 + 1500), //yaw
      	     (int)Math.round(GamePad.getAxis(GamePad.THROTTLE) * 1000 + 1000), //throttle
      	     GamePad.AUX1.getVal() ? 2000 : 1000, //AUX 1-4 V 
  	         GamePad.AUX2.getVal() ? 2000 : 1000,
             GamePad.AUX3.getVal() ? 2000 : 1000,
      	    enabled ? 2000 : 1000
      	};
      	List<Character> realPayload = new LinkedList<Character>();
      	for(int num : rcPayload) {
      	  realPayload.add((char) (num & 0xff));
      	  realPayload.add((char) ((num >> 8) & 0xff));
      	}
 
      	connection.sendRcPayload(realPayload.toArray(new Character[0]));
      } else {
        enabled = false;
        System.out.println("NO GAMEPAD!");
      }
      if ((time-time2)>101  ){
        time2=time;
 ...
 if(ServosActive){
   ...
   //******************************************************************************************
    ...
    if (toggleWrite) {
      ...
      byteP[4] = ((int) Math.round(confP[4].value()*100.0));
      byteI[4] = ((int) Math.round(confI[4].value()*100.0));

      byteP[5] = ((int) Math.round(confP[5].value()*10.0));
      byteI[5] = ((int) Math.round(confI[5].value()*100.0));
      byteD[5] = ((int) Math.round(confD[5].value()*10000.0))/10;

      byteP[6] = ((int) Math.round(confP[6].value()*10.0));
      byteI[6] = ((int) Math.round(confI[6].value()*100.0));
      byteD[6] = ((int) Math.round(confD[6].value()*10000.0))/10;
    ...
    int orig_avail = g_serial.available();
 //   System.out.println("AVAILABLE: " + orig_avail);
...
//        SaveSERVO.hide();
  ...
  fill(255,255,255);
  pushMatrix();
  camera(xObj,yObj,(float) (300/Math.tan(PI*60.0/360.0)),xObj/2+30,yObj/2-40,0,0,1,0);
  translate(xObj,yObj);
...
void ReadSerialPort() {
  ...
  if (line == null) {
   ...
   return;
    ...
    int Port = Integer.parseInt(pieces[0]);
    GUI_BaudRate= Integer.parseInt(pieces[1]);
    if (commListMax==1){}
